...
class AnalyzeResponse(BaseModel):
    product_name: str
    advice: str
    source: str   # "cache" | "groq" | "rate_limited"
    cache_hit: bool
    rate_limited: bool
...
async def analyze_all(db: Session = Depends(get_session)):
    ...
    if not all_products:
        raise HTTPException(
            ...
            detail="No products were found in the database.",
        )
    if len(all_products) > 20:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="A maximum of 20 products is supported for bulk analysis.",
        )
    ...
    for p in all_products:
        r = get_advice_with_protection(
            ...
            avg_sales=p["avg_daily_sales"],
            _ai_fn=get_ai_advice,
        )
...
@router.get(
    ...
    description="It retrieves product data from the database and analyzes it using the configured AI service.",
)
async def analyze_by_id(
    ...
    result = get_advice_with_protection(
        ...
        avg_sales=data["avg_daily_sales"],
        _ai_fn=get_ai_advice,
    )
